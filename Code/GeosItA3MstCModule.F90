...
!BOP
!
! !MODULE: GeosItA3MstCModule
!
! !DESCRIPTION: Module GeosItA3MstCModule contains routines to create the
!  GEOS-Chem average 3-hr data files (moist parameters on level centers)
!  from the GEOS-IT raw data.
!\\
!\\
! !INTERFACE:
...
100 FORMAT ( '%%% Defining netCDF file vars & attrs for ', a' grid' )
    ...
    ! Title string
    lName = 'GEOS-IT time-averaged 3-hour moist parameters on level centers (A3mstC), processed for GEOS-Chem input'
    ...
    ! DQRCU
    IF ( StrPos( 'DQRCU', mst_tavg_3hr_v72_data ) >= 0 ) THEN
    ...
    ! DQRLSAN
    IF ( StrPos( 'DQRLSAN', mst_tavg_3hr_v72_data ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idLev, idTime /)
    ...
    ! REEVAPCN
    IF ( StrPos( 'REEVAPCN', mst_tavg_3hr_v72_data ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idLev, idTime /)
    ...
    ! REEVAPLS(AN)
    ! Rename raw data variable REEVAPLSAN to REEVAPLS for use in GEOS-Chem
    IF ( StrPos( 'REEVAPLSAN', mst_tavg_3hr_v72_data ) >= 0 ) THEN
       var4  = (/ idLon, idLat, idLev, idTime /)
...
!BOP
!
! !IROUTINE: GeosItMakeA3MstC
!
! !DESCRIPTION: Routine GeosItMakeA3MstC is the the driver routine for
! \begin{enumerate}
! \item Extracting 3-hr time-averaged data fields (moist parameters on level
!       centers) from the GEOS-IT raw data files (netCDF-4 format),
...
!
    ...
    ! Echo info
    msg = '%%%%%%%%%% ENTERING ROUTINE GeosItMakeA3MstC %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' )
    WRITE( IU_LOG, '(a)' ) TRIM( msg )

    ! List of all the A-3 fields combined
    allFieldsList = TRIM( mst_tavg_3hr_v72_data )

    ! Return the list of fields and number of fields to process
    ! from each of the GeosIt raw met data files
    CALL GetNFields( mst_tavg_3hr_v72_data,  nFields_3dMstNv, fields_3dMstNv )
    CALL GetNFields( allFieldsList,         nAllFields,      allFields      )

    ! Echo info
    WRITE( IU_LOG, 100 ) TRIM( mst_tavg_3hr_v72_file ), nFields_3dMstNv
...
110 FORMAT( '%%% TOTAL # OF FIELDS TO BE REGRIDDED : ', i5 )
   ...
   ENDIF
    ...
    ! Open nested 0.5x0.625 CH output file
    IF ( doNestCh05 ) THEN
    ...
    ! Open nested 0.5x0.625 EU output file
    IF ( doNestEu05 ) THEN
    ...
    ! Open nested 0.5x0.625 NA output file
    IF ( doNestNa05 ) THEN
    ...
    ! Open nested 0.5x0.625 SE output file
    IF ( doNestSe05 ) THEN
    ...
    ! Open nested 0.5x0.625 IN output file
    IF ( doNestAs05 ) THEN
    ...
    ! Process data
    !=======================================================================
    CALL Process3dMstNv( nFields_3dMstNv, fields_3dMstNv ) ! mst_tavg_3hr_v72
    ...
    ! Echo info
    msg = '%%%%%%%%%% LEAVING ROUTINE GeosItMakeA3MstC %%%%%%%%%%'
    WRITE( IU_LOG, '(a)' ) '%%%'
    WRITE( IU_LOG, '(a)' ) TRIM( msg )

  END SUBROUTINE GeosItMakeA3MstC
!EOC
!------------------------------------------------------------------------------
!          Harvard University Atmospheric Chemistry Modeling Group            !
!------------------------------------------------------------------------------
...
!
! !DESCRIPTION: Subroutine Process3dMstNv regrids the GeosIt met fields from
!  the "mst\_tavg\_3d\_v72" file and saves output to netCDF file format.
!\\
!\\
! !INTERFACE:
!
...
!
   ...
   ENDIF
    ...
    DO H = 1, TIMES_A3
       ...
       fNameInput = TRIM( inputDataDir ) // TRIM( mst_tavg_3hr_v72_file )
       ...
       DO F = 1, nFields
          ...
          IF ( do05x0625 ) THEN
            ...
            CALL NcWr( Ptr, fOut05x0625, TRIM( name8 ), st4d, ct4d )
            NULLIFY( Ptr )
          ENDIF
          ...
          IF ( doNestCh05 ) THEN
             ...
             CALL NcWr( Ptr, fOut05NestCh, TRIM( name8 ), st4d, ct4d )
             NULLIFY( Ptr )
          ENDIF
          ...
          IF ( doNestSe05 ) THEN
             ...
             CALL NcWr( Ptr, fOut05NestSe, TRIM( name8 ), st4d, ct4d )
             NULLIFY( Ptr )
          ENDIF
